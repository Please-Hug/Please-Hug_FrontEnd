    ...
    <>
      ...
      <div className={styles.overlay}>
        ...
        <div className={styles.modal}>
          {showReceiverAlert && (
            <div className={styles.alert}>대상을 선택해주세요.</div>
          )}
          {showContentAlert && (
            <div className={styles.alert}>전달할 메시지를 적어주세요.</div>
          )}
          ...
          <div className={styles.inputWrapper}>
            {receivers.map((r) => (
              <span key={r.username} className={styles.selectedTag}>
                ...
                <button
                  onClick={() =>
                    setReceivers(
                      receivers.filter((u) => u.username !== r.username)
                    )
                  }
                >
                  ✕
            ...
            <input
              ...
              placeholder={
                receivers.length === 0 ? "구성원 또는 그룹 선택" : ""
              }
            />

            {receivers.length > 0 && (
              <button
                onClick={handleResetReceivers}
